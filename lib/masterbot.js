...
exports.init = _ => {
  if (process.env.NODE_ENV !== 'development' || (config.bot && config.bot.token && config.bot.bid)) {
    const bid = process.env.POLY_BID || config.bot.bid
    const token = process.env.POLY_TOKEN || config.bot.token

    if (!bid || !token) return console.log('[ERROR] init: bid and token are required')
